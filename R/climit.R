climit <- function(imgs, fa.rate, model, type,
                   stat = c("ad", "bp"), w, nD = 10, verbose = FALSE)
{
  if (verbose) {
  ptm <- proc.time()
cat("Computing control limits... ")
  ...
  if (2 %in% type) {
    ...
    if (b >=0)
      ...
      globalStat$control.limit.trans_chi2 <- qchisq(fa.rate, k)*b + a
  ...
  out <- list(type = type, fa.rate = fa.rate,
              localStat = localStat, globalStat = globalStat)
  class(out) <- "climit"
